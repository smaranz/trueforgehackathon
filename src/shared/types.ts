...
export interface AuditState { model: string; requestedAgents: number; concurrency: number; maxStepsPerAgent: number; deadlineMinutes: number; maxTotalTokens: number | null; stage: 'registration' | 'exploration' | 'verification' | 'complete'; agents: AuditAgent[]; reproductions: AuditReproduction[]; accountCount: number; }
export interface AuditInput { targetUrl: 'http://localhost:3000/signup'; agentCount: number; concurrency: number; maxStepsPerAgent: number; deadlineMinutes: number; /** Legacy input accepted for compatibility; new runs always have no total-token cutoff. */ maxTotalTokens?: number | null; goal: string; }
