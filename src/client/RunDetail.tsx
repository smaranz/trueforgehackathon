...
function FindingsList({ run }: { run: Run }) {
  return <section className="findings-section"><SectionTitle title="Findings"><span className="count-badge mono">{run.findings.length}</span></SectionTitle>{run.findings.length ? <div className="findings-list">{run.findings.map((finding, index) => <a className="finding-card" href={runHash(run.id, finding.id)} key={finding.id}><div className="finding-card-top"><span className="eyebrow">FINDING {String(index + 1).padStart(2, '0')}</span><Status value={finding.severity} dot={false} /></div><h3>{finding.title}</h3><p>{finding.actual}</p><div className="finding-card-bottom"><Status value={finding.status} /><ArrowRight size={18} /></div></a>)}</div> : <EmptyState icon={<FileMagnifyingGlass size={25} />} title={isActive(run.status) ? 'Evidence before conclusions.' : 'No findings recorded'}>{isActive(run.status) ? 'Findings appear when there is something to inspect. Nothing is assumed.' : run.status === 'completed' ? 'This run completed without a reported finding. Review the timeline for the checks performed.' : 'Check the run status and timeline for what happened.'}</EmptyState>}
