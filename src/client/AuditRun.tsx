...
function AgentInspector({ agent, run }: { agent: AuditAgent; run: Run }) {
  ...
  return <>
    ...
    <div className="audit-inspector-body">
      ...
      {agent.error && <div className="audit-agent-error" role="status"><WarningCircle size={16} /><p>{agent.error}</p></div>}
      ...
      {findings.length > 0 && <div className="audit-agent-findings"><span className="audit-label">THIS AGENT’S FINDINGS</span>{findings.map(finding => <a key={finding.id} href={runHash(run.id, finding.id)}>{finding.title}<ArrowRight size={14} /></a>)}</div>}
    </div>
  </>;
}
...
function AgentPool({ run }: { run: Run }) {
  ...
  return <section className="audit-pool-section" aria-labelledby="audit-pool-title">
    ...
    <div className="audit-pool-layout">
      ...
      <div className="audit-inspector" id="audit-agent-inspector" ref={inspector} tabIndex={-1} aria-label={selected ? `Details for ${selected.name}` : 'Agent inspector'}>{selected ? <AgentInspector key={selected.id} agent={selected} run={run} /> : <div className="audit-inspector-empty"><FileMagnifyingGlass size={30} /><h3>Your evidence, up close.</h3><p>Select an available agent to inspect their session, signup outcome, and recorded coverage.</p></div>}</div>
    </div>
  </section>;
}
...
function TimelineEvent({ event, run }: { event: ActionEvent; run: Run }) {
  ...
  return <details className="audit-event" onToggle={change => setOpen(change.currentTarget.open)}><summary><time dateTime={event.timestamp} title={dateTime(event.timestamp)}>{clock(event.timestamp)}</time><span className="audit-event-node" /><span className="audit-event-content"><span className="audit-event-labels"><strong>{agentName(run, event.actorId)}</strong><span>{event.type}</span></span><span className="audit-event-message">{event.message}</span></span><CaretRight size={13} className="audit-chevron" /></summary>{open && <div className="audit-event-body"><dl className="audit-facts"><div><dt>Phase</dt><dd>{event.phase}</dd></div><div><dt>Timestamp</dt><dd><RecordedTime value={event.timestamp} /></dd></div><div className="audit-fact-wide"><dt>Event ID</dt><dd><code>{event.id}</code></dd></div>{event.actorId && <div className="audit-fact-wide"><dt>Agent ID</dt><dd><code>{event.actorId}</code></dd></div>}{event.sessionId && <div className="audit-fact-wide"><dt>Session ID</dt><dd><code>{event.sessionId}</code></dd></div>}</dl>{event.details !== undefined && event.details !== null ? <pre>{serialize(event.details)}</pre> : <p className="audit-help">No additional tool details were recorded.</p>}{event.artifactIds.length > 0 && <><ArtifactList artifacts={run.artifacts.filter(artifact => event.artifactIds.includes(artifact.id))} run={run} />{event.artifactIds.some(id => !run.artifacts.some(artifact => artifact.id === id)) && <p className="audit-help">Some referenced artifacts are not available in this run.</p>}</>}</div>}</details>;
}
...
function AuditWorkspace({ run, onCancel, cancelling, connection, actionError }: AuditRunProps) {
  ...
  return <div className="audit-page audit-run">
    ...
    {actionError && <ErrorNotice>{actionError}</ErrorNotice>}{run.error && <ErrorNotice>{run.error}</ErrorNotice>}{exportError && <ErrorNotice>{exportError}</ErrorNotice>}
    ...
    <dl className="audit-metrics">
      ...
      <div><dt>Tokens used</dt><dd className="audit-metric-tokens">{tokens === undefined ? 'Not reported' : tokens.toLocaleString()}</dd><p>{audit ? `${audit.maxTotalTokens.toLocaleString()} budget` : 'Budget unknown'}</p></div>
