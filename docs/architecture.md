...
- **`coordinator.ts`** queues specialists, serializes registrations and streams real turns. Defaults: 30 agents, four concurrent, 60 steps each, 60 minutes, unlimited total tokens. Reported token totals remain observable but do not control scheduling, rechecks or reproduction. New runs persist a null token limit; numeric limits remain meaningful only in historical reports. Real timestamps determine duration without artificial time-filling delays. Provider429 errors use cancellable shared backoff and same-session continuation; successful browser writes are not automatically repeated.
